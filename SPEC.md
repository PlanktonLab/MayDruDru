...
| D45 | apply-web 提供獨立 `/demo` 一頁式產品介紹：手機優先、白色簡約排版；資料去敏沿用正式遮罩編輯器，SOP 展示與聊天選單讀取公開 API 中國泰世華已發布流程，文件標注與 LINE 對話使用前端可操作展示；SOP API 離線時明示為離線展示內容 | 評審可在單一網址理解並實際操作四項能力；重用正式資料與遮罩元件避免舞台 Demo 和產品走出兩套行為，離線備援則避免環境尚未啟動時整頁失效或把靜態圖誤稱為資料庫內容 |
